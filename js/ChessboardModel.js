  ...
  var ChessboardModel = Backbone.Model.extend({
    ...
    hasAnyRowConflict: function(){
      ...
      for (var i = 0; i < this.attributes.board.length; i++) {
        if (this.hasRowConflictAt(this.attributes.board[i])){
          hasConflict = true ;
        } 
      }
      return hasConflict;
    ...
    hasRowConflictAt: function(r){
      ...
      for (var i = 0; i < r.length; i++) {
        if(r[i].piece  === true) {truecount++;}
      }
      if (truecount > 1) {return true;} else {return false;}
    },

    hasAnyColConflict: function(){
      var hasConflict = false;
      for (var i in this.get('board') ){
        var colArray = [];
        for (var j in this.get('board')[i]){
          colArray.push(this.get('board')[j][i].piece)
        }
        // console.log(colArray);
      return this.hasColConflictAt(colArray);
      }
    },
    hasColConflictAt: function(c){
      ...
      for (var i = 0; i < c.length; i++) {
        console.log(c[i]);
        if(c[i]  === true) {hasAProblem++;}
      }
      if (hasAProblem > 1) {return true;} else {return false;}
        ...
        for ( var j in board[i]){
          if (board[i][j].piece == true){
            ...
            for (var x in board){
              ...
              if (board[i2]){
                if(board[i2][j2]){
                  if (board[i2][j2].piece) {truthy = true;}
    ...
    hasUpLeftConflictAt: function(upLeftIndex){
      return this.hasAnyUpLeftConflict();
    },
    ...
    hasUpRightConflictAt: function(upRightIndex){
      return this.hasAnyUpRightConflict();
    }
  });
